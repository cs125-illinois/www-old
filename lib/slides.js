...
exports = module.exports = (config) => {
  return (files, metalsmith, done) => {
    ...
    _.each(slides, (file, filename) => {
      ...
      let dateSlug = `${ moment(file.date).format('YYYY_MM_DD') }_${ rawSlug }`
      ...
      file.semester = _.findKey(metadata.config.semesters, semester => {
        return moment(file.date)
          .isBetween(moment(semester.start, 'MM-DD-YYYY'),
          moment(semester.end, 'MM-DD-YYYY'))
      })
