...
exports = module.exports = (config) => {
  return (files, metalsmith, done) => {
    ...
    _.each(slides, (file, filename) => {
      ...
      expect(slideIDs, file.uuid).to.not.have.property(file.uuid)
      ...
      $('div.janini pre').each((i, element) => {
        ...
        section.replaceWith(() => {
          return `<textarea class="janini spelling_exception">${ $(element).text() }</textarea>` +
            `<!-- beautify ignore:start --><div class="output" data-blank="${ message }"><pre style="color: white;">${ message }</pre></div><!-- beautify ignore:end -->`
        })
      })
