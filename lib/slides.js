  ...
  return (files, metalsmith, done) => {
    ...
    _.each(slides, (file, filename) => {
      ...
      $('h2').replaceWith((i, element) => {
        ...
        expect(slideID, file.filename).to.be.ok
    ...
    slides= _(slides).sortBy('date').reverse().value()

    let metadata = metalsmith.metadata()
    metadata.slides = {
      main: _.filter(slides, s => { return !s.emp }),
      emp: _.filter(slides, s => { return s.emp })
    }
    console.log(metadata.slides.emp.length)
