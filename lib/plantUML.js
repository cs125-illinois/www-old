...
@startuml
digraph G {
  node [ fontname="Open Sans", margin="0.05,0.03", fontsize=12, shape="record", color="#e84a27" ]
...
let doPlantUML = async (file, filename, config, metadata) => {
  ...
  for (let diagram of diagrams) {
    ...
    if ($(diagram).hasClass('digraph') && $(diagram).hasClass('default')) {
      html = defaultDigraphTemplate({ html, rankdir })
    } else if ($(diagram).hasClass('digraph') && $(diagram).hasClass('small')) {
      html = smallDigraphTemplate({ html, rankdir })
    } else if ($(diagram).hasClass('digraph') && $(diagram).hasClass('smaller')) {
      html = smallerDigraphTemplate({ html, rankdir })
    }
    ...
    if (!result) {
      try {
        ...
        svgDOM('svg *').each((i, element) => {
          let existingID = svgDOM(element).attr('id')
          if (existingID) {
            svgDOM(element).attr('id', `${ existingID }_${ randomSuffix }`)
          }
        })
        result = svgDOM.html()
      } catch (err) {
