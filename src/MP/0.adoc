...
your "Test Library" run configuration.

Click the dropdown next to the play button and choose "Edit Configurations".
//
Choose "Test Library".
//
You want the bottom to look like this:

image::CleanBeforeBuild.png[role='img-fluid']

**If it doesn't**, click on the plus button shown in the image above and, choose
"Gradle-aware Make", and type ":lib:clean".
//
Then make sure that item is first in the list as shown above.
//
