...
==== "No tests were found" (1/2)

If you see the following error message when you run the test suites:

image::NoTest.png[role='img-fluid']

_and_ you downloaded the MP right after it was released, you may need to adjust
your run configurations.
//
Here's how to do that:

. Switch the left pane to Project view
//
. Expand the top (`MP0-Spring2019-`...) folder, then `.preserve`, then
`runConfigurations`
//
. Double-click `Test_App.xml` to open it
//
Position your cursor after `<method v="2">` (line 15), press enter to insert a
new line, and then paste this text:
//
[source,xml]
----
<option name="Android.Gradle.BeforeRunTask" enabled="true" goal=":lib:clean" />
----
//
[start=4]
//
. Repeat steps 4 and 5 for `Test_Library.xml`
//
Press the Make button (green hammer) in the toolbar

Or, you could just watch Ben show you how to do it:

++++
<div class="row justify-content-center mt-3 mb-3">
  <div class="col-12 col-lg-8">
    <div class="embed-responsive embed-responsive-4by3">
      <iframe class="embed-responsive-item" width="560" height="315" src="//www.youtube.com/embed/xC-9TelgnWk" allowfullscreen></iframe>
    </div>
  </div>
</div>
++++

==== "No tests were found" (2/2)

Even if you've completed the steps shown above you can still encounter this
message.
//
This is usually because your code doesn't
_compile_ (usually due to a syntax error) and so can't be tested.
//
When you try to run the test suite or an individual test you see this:

image::NoTest.png[role='img-fluid']

However, see the "Build" tab down there a few over from the "Run" tab that is
currently active?
//
That's what you want to look at, since it will show build output identifying the
error:

image::ShowTest.png[role='img-fluid']
